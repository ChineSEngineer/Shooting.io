...
var lp = setInterval(function(){httpGetAsync("/data", callback);}, 50);
...
// setInterval(function(){callback("123");}, 1000);
...
function parse(jstr) {
    const state = JSON.parse(jstr);
    drawGameBoard(20);
    for (let player of state['players']) {
        placeCircle(player['x'], player['y'], '#ffff00', 2.0);
    }
    for (let bullet of state['bullets']) {
        placeCircle(bullet['x'], bullet['y'], 'red', 1.0);
    }
